...
>
> ```bash
> uv run --no-project --with 'fused[vector]' fused workbench run my_canvas my_udf --param=value
> ```
>
...
```bash
uv run --with 'fused[vector]' fused workbench run my_canvas my_udf --param=value
```
